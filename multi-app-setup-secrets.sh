...
# Show a value for confirmation (mask passwords, tokens, and API keys)
show_value() {
    local name="$1"
    local value="$2"
...
show_requirements() {
    ...
    echo "from separate upstream repos with independent release tags,"
    echo "with Nonprod/Prod foundations and an approval gate for production."
    echo ""
    ...
    print_bullet "APP1_UPSTREAM_REPO        - Source repo (e.g., org/my-api)"
    print_bullet "APP1_NAME                 - CF app base name"
    print_bullet "APP1_MANIFEST_PATH        - Path to manifest in this repo"
    print_bullet "APP1_ARTIFACT_PATTERN     - Release asset pattern"
    print_bullet "APP1_CF_ENV_JSON          - (Optional) JSON env vars for cf set-env"
    echo ""
    echo -e "${CYAN}Application 2:${NC}"
    print_bullet "APP2_UPSTREAM_REPO        - Source repo (e.g., org/my-worker)"
    print_bullet "APP2_NAME                 - CF app base name"
    print_bullet "APP2_MANIFEST_PATH        - Path to manifest in this repo"
    print_bullet "APP2_ARTIFACT_PATTERN     - Release asset pattern"
    print_bullet "APP2_CF_ENV_JSON          - (Optional) JSON env vars for cf set-env"
    echo ""
...
main() {
    ...
    echo -e "${CYAN}Application 1:${NC}"
    show_value "APP1_UPSTREAM_REPO" "$APP1_UPSTREAM_REPO"
    ...
    show_value "APP1_CF_ENV_JSON" "$APP1_CF_ENV_JSON"
    echo ""
    echo -e "${CYAN}Application 2:${NC}"
    show_value "APP2_UPSTREAM_REPO" "$APP2_UPSTREAM_REPO"
    ...
    show_value "APP2_CF_ENV_JSON" "$APP2_CF_ENV_JSON"
    ...
    # App 1 secrets
    set_secret "APP1_UPSTREAM_REPO" "$APP1_UPSTREAM_REPO" && ((success_count++)) || true
    ...
    set_secret "APP1_CF_ENV_JSON" "$APP1_CF_ENV_JSON" && ((success_count++)) || true

    # App 2 secrets
    set_secret "APP2_UPSTREAM_REPO" "$APP2_UPSTREAM_REPO" && ((success_count++)) || true
    ...
    set_secret "APP2_CF_ENV_JSON" "$APP2_CF_ENV_JSON" && ((success_count++)) || true
    ...
    # Save debug file with all configured values (sensitive values masked)
    local debug_file=".multi-app-secrets-debug.txt"
    {
        echo "# MCP App Group 1 - Secrets Configuration Debug File"
        ...
        echo "## Application 1"
        echo "APP1_UPSTREAM_REPO=${APP1_UPSTREAM_REPO}"
        echo "APP1_NAME=${APP1_NAME}"
        echo "APP1_MANIFEST_PATH=${APP1_MANIFEST_PATH}"
        echo "APP1_ARTIFACT_PATTERN=${APP1_ARTIFACT_PATTERN}"
        [ -n "$APP1_CF_ENV_JSON" ] && echo "APP1_CF_ENV_JSON=********" || echo "APP1_CF_ENV_JSON=(not set)"
        echo ""
        echo "## Application 2"
        echo "APP2_UPSTREAM_REPO=${APP2_UPSTREAM_REPO}"
        echo "APP2_NAME=${APP2_NAME}"
        echo "APP2_MANIFEST_PATH=${APP2_MANIFEST_PATH}"
        echo "APP2_ARTIFACT_PATTERN=${APP2_ARTIFACT_PATTERN}"
        [ -n "$APP2_CF_ENV_JSON" ] && echo "APP2_CF_ENV_JSON=********" || echo "APP2_CF_ENV_JSON=(not set)"
        echo ""
        if [ "$CONFIGURE_SHARED" = "true" ]; then
            echo "## Nonprod CF Foundation"
            echo "CF_NONPROD_API=${CF_NONPROD_API}"
            echo "CF_NONPROD_USERNAME=${CF_NONPROD_USERNAME}"
            echo "CF_NONPROD_PASSWORD=********"
    ...
    print_warning "Add ${debug_file} to .gitignore (contains non-secret config values)"
