...
compute_names() {
    ...
    DISPLAY_NAME="Deploy ${WORKFLOW_NAME}"
...
# Generated by multi-app-setup-secrets.sh on $(date -u '+%Y-%m-%d')
...
generate_manifests() {
    ...
    for i in $(seq 0 $((NUM_APPS - 1))); do
        local manifest_path="${APP_MANIFEST_PATHS[$i]}"
...
main() {
    ...
    for i in $(seq 0 $((NUM_APPS - 1))); do
        ...
        if [ "$manifest_choice" = "2" ]; then
            ...
            APP_MANIFEST_PATHS[$i]="manifests/${APP_NAMES[$i]}/manifest.yml"
        fi
        echo ""
    ...
    for i in $(seq 0 $((NUM_APPS - 1))); do
        ...
        if [ "${APP_MANIFEST_MODES[$i]}" = "byom" ]; then
            show_value "Manifest mode" "bring your own"
        else
    ...
    # Generate the workflow and manifests
    ...
    for i in $(seq 0 $((NUM_APPS - 1))); do
        if [ -f "${APP_MANIFEST_PATHS[$i]}" ]; then
            echo -e "  ${CYAN}${APP_MANIFEST_PATHS[$i]}${NC}"
        fi
    done
    echo ""
    ...
    # Build manifest paths list and git add paths
    local manifest_list=""
    local git_add_paths="${GENERATED_WORKFLOW}"
    for i in $(seq 0 $((NUM_APPS - 1))); do
        [ -n "$manifest_list" ] && manifest_list="${manifest_list}\n"
        manifest_list="${manifest_list}   ${DIM}${APP_MANIFEST_PATHS[$i]}${NC}"
        git_add_paths="${git_add_paths} ${APP_MANIFEST_PATHS[$i]}"
    done

    echo "1. Review and commit the generated files:"
    echo -e "   ${DIM}git add ${git_add_paths}${NC}"
    echo -e "   ${DIM}git commit -m \"Add deployment workflow and manifests\"${NC}"
    ...
    echo ""
    # Build separate lists for generated vs BYOM manifests
    local generated_list=""
    local byom_list=""
    for i in $(seq 0 $((NUM_APPS - 1))); do
        if [ "${APP_MANIFEST_MODES[$i]}" = "byom" ]; then
            [ -n "$byom_list" ] && byom_list="${byom_list}\n"
            byom_list="${byom_list}   ${DIM}${APP_MANIFEST_PATHS[$i]}${NC}"
        else
            [ -n "$generated_list" ] && generated_list="${generated_list}\n"
            generated_list="${generated_list}   ${DIM}${APP_MANIFEST_PATHS[$i]}${NC}"
        fi
    done
    echo "5. Review the CF manifest files:"
    if [ -n "$generated_list" ]; then
        echo -e "   ${CYAN}Generated (customize as needed):${NC}"
        echo -e "$generated_list"
    fi
    if [ -n "$byom_list" ]; then
        echo -e "   ${CYAN}Bring your own (ensure these exist):${NC}"
        echo -e "$byom_list"
    fi
    echo ""
    if [ "$GITHUB_PLATFORM" = "ghe" ]; then
